...
#define CONFIG_VALUE_MSB 0x42 // 0000 1100 // CONFIG ADS1115   TO BE IN coninuous mode
#define CONFIG_VALUE_LSB 0x03   //1000 0011
...
static void i2cTask_(void *arg)
{
    ...
    while (true)
    {
        ...
        {
            ESP_LOGI(TAG, "*******************\n");
            an0 = (int16_t)(((uint16_t)sensor_data[0] << 8) | (uint16_t)sensor_data[1]);
            dec1 = (an0 / 790) / 10;
            dec0 = (((int)an0 / 790) % 10);

            ESP_LOGI(TAG, "Sensor Value: %d", (int16_t)an0);
            ESP_LOGI(TAG, "Voltage: %d.%d", dec1, dec0);
        }
        else
        {
